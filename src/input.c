                ...
                if (ui->placing_circuit && ui->selected_circuit_type > 0) {
                    if (!input->sim_running) {
                        ...
                        if (count > 0) {
                            ...
                            { int fl = circuit_template_scope_flags(ui->selected_circuit_type); ui->scope_ac_coupling = (fl & SCOPE_FLAG_AC) != 0; ui->scope_stacked = (fl & SCOPE_FLAG_STACK) != 0; ui->scope_stack_fit = (fl & SCOPE_FLAG_FIT) != 0; ui->trigger_mode = (fl & SCOPE_FLAG_SINGLE) ? TRIG_SINGLE : TRIG_AUTO; }

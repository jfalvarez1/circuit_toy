...
/* ---- current direction, as a law ------------------------------------------------------- */

/* A passive part cannot generate power. With the terminal-current convention this program uses
   - positive current ENTERS terminal 0 - that is simply I*(V0 - V1) >= 0, and it holds for every
   resistor, diode, LED, lamp and fuse on every template, at every operating point, for ever.

   This is deliberately a law and not a table of expected directions. --sign-test checks four
   sources and two transformers against hand-worked expectations, which is worth having and does
   not scale: it says nothing about the 199 other templates, and nothing at all about a template
   or a part added next week. An inequality that must hold everywhere needs no maintenance and
   covers what does not exist yet.

   Capacitors and inductors are excluded on purpose - a storage element returns energy, so
   negative instantaneous power is exactly what it is supposed to do. Sources are excluded for
   the same reason in reverse. */
/* 2 = purely dissipative, and so bound by the law at every instant. 1 = a junction, which
   stores charge: the displacement current into a reverse-biased junction is real current in the
   wire and it RETURNS energy, so instantaneous V*I below zero is what a charged junction is
   supposed to do. Those are held to the law at the operating point instead, where there is no
   dv/dt and a diode conducting backwards has nowhere to hide. */
static int direction_is_passive(ComponentType t) {
    switch (t) {
        case COMP_RESISTOR: case COMP_LAMP: case COMP_FUSE:
        case COMP_THERMISTOR: case COMP_PHOTORESISTOR: case COMP_POTENTIOMETER:
            return 2;
        case COMP_DIODE: case COMP_LED: case COMP_ZENER: case COMP_SCHOTTKY:
            return 1;
        default: return 0;
    }
}

static int direction_check(Circuit *c, int min_class, char *why, size_t nwhy) {
    for (int i = 0; i < c->num_components; i++) {
        Component *comp = c->components[i];
        if (!comp || comp->num_terminals != 2) continue;
        if (direction_is_passive(comp->type) < min_class) continue;
        Node *a = circuit_get_node(c, comp->node_ids[0]);
        Node *b = circuit_get_node(c, comp->node_ids[1]);
        if (!a || !b) continue;
        double v = a->voltage - b->voltage;
        double I = comp->terminal_current[0];
        if (!isfinite(v) || !isfinite(I)) continue;
        double p = v * I;
        /* The floor is what a cancellation of two large nearly-equal terms costs: a part
           carrying amps at millivolts has its power recovered from a stamp residual, and Newton's
           own slack lands there. A reversed current is not small - it is the whole power with a
           minus sign in front. */
        double floor = 1e-6 * fabs(v * I) + 1e-9 + 1e-7 * fabs(I) + 1e-7 * fabs(v);
        if (p < -floor) {
            snprintf(why, nwhy, "%s has %.4g V across it and %.4g A entering terminal 0, which is "
                     "%.3g W generated by a part that cannot generate", comp->label, v, I, -p);
            return 0;
        }
    }
    return 1;
...
static int direction_test(void) {
    printf("direction-test: a passive part cannot generate power, on every template\n\n");
    int fails = 0, total = 0, parts = 0;
    for (int t = CIRCUIT_NONE + 1; t < CIRCUIT_TYPE_COUNT; t++) {
        if (shard_skip(t)) continue;
        const CircuitTemplateInfo *ti = circuit_template_get_info((CircuitTemplateType)t);
        const char *name = ti ? ti->name : "?";
        ...
        if (circuit_place_template(c, (CircuitTemplateType)t, 0, 0) <= 0) { circuit_free(c); continue; }
        for (int i = 0; i < c->num_components; i++)
            if (c->components[i] && direction_is_passive(c->components[i]->type)) parts++;
        Simulation *sim = simulation_create(c);
        total++;
        char why[220] = "";
        int ok = 1;
        if (!sim || !simulation_dc_analysis(sim)) { simulation_free(sim); circuit_free(c); continue; }
        simulation_update_flow_display(sim);
        ok = direction_check(c, 1, why, sizeof why);   /* everything, at the operating point */
        if (ok) {
            /* and again with the circuit running, where a sign error in a companion shows and a
               DC operating point cannot */
            simulation_auto_time_step(sim);
            simulation_start(sim);
            double t_end = sim->time + 200.0 * sim->time_step;
            while (sim->time < t_end) {
                if (!simulation_step(sim)) break;
                simulation_update_flow_display(sim);
                if (!direction_check(c, 2, why, sizeof why)) { ok = 0; break; }
            }
        }
        if (!ok) { fails++; printf("[FAIL] dir   %-28s %s\n", name, why); }
        simulation_free(sim);
        circuit_free(c);
    }
    printf("\ndirection-test: %d templates, %d passive parts watched at the operating point and "
           "through 200 steps, %d generating power\n", total, parts, fails);
    return fails ? 1 : 0;
}

/* ---- thermal overload ------------------------------------------------------------------- */

/* A part driven past its rating has to say so, and a part inside its rating must not. Both
   directions: a warning that never fires and one that always fires are equally useless.

   The rating used has to be the PART'S. That was the bug this suite was written for - the damage
   model had a flat 0.25 W for every resistor and never looked at props.resistor.power_rating,
   while the overload warning in render.c has always divided by exactly that. The two disagreed,
   so a 5 W resistor at 1 W drew no warning and took damage anyway. */
static Circuit *thermal_rig(double r_ohm, double v_supply, double rating_w) {
    Circuit *c = circuit_create();
    if (!c) return NULL;
    Component *src = component_create(COMP_DC_VOLTAGE, -200, 0);
    src->props.dc_voltage.voltage = v_supply;
    circuit_add_component(c, src);
    Component *r = component_create(COMP_RESISTOR, 100, 0);
    r->props.resistor.resistance = r_ohm;
    r->props.resistor.power_rating = rating_w;
    circuit_add_component(c, r);
    Component *g = component_create(COMP_GROUND, -200, 200);
    circuit_add_component(c, g);
    circuit_add_wire(c, src->node_ids[0], r->node_ids[0]);
    circuit_add_wire(c, r->node_ids[1], g->node_ids[0]);
    circuit_add_wire(c, src->node_ids[1], g->node_ids[0]);
    return c;
}

static int thermal_test(void) {
    printf("thermal-test: a part over its rating burns, a part inside it does not\n\n");
    int fails = 0, total = 0;

    /* R, V, rating, expect damage, why */
    static const struct {
        double r, v, rating;
        int expect_burn;
        const char *what;
    } cases[] = {
        { 100.0,  10.0, 0.25, 1, "1 W into a quarter-watt resistor: four times its rating" },
        { 100.0,  10.0, 5.00, 0, "the same 1 W into a 5 W resistor: well inside it, and the "
                                 "damage model has to read the rating to know that" },
        { 1000.0, 10.0, 0.25, 0, "100 mW into a quarter-watt resistor: inside its rating" },
        { 10.0,   10.0, 0.25, 1, "10 W into a quarter-watt resistor: forty times over" },
        { 1000.0,  1.0, 0.25, 0, "1 mW: nowhere near" },
        { 10.0,   10.0, 100.0, 0, "10 W into a 100 W part: a rating is a rating" },
    };

    for (int i = 0; i < (int)(sizeof cases / sizeof cases[0]); i++) {
        total++;
        Circuit *c = thermal_rig(cases[i].r, cases[i].v, cases[i].rating);
        if (!c) continue;
        Simulation *sim = simulation_create(c);
        int ok = 1;
        char why[200] = "";
        if (!sim || !simulation_dc_analysis(sim)) { ok = 0; snprintf(why, sizeof why, "DC failed"); }
            ...
            simulation_set_time_step(sim, 1e-3);
            simulation_start(sim);
            /* long enough for damage to accumulate: the model integrates it over time */
            for (int s = 0; s < 4000 && sim->time < 4.0; s++) if (!simulation_step(sim)) break;
            ...
            if (fabs(p - expect_p) > 0.05 * expect_p + 1e-6) {
                ok = 0;
                snprintf(why, sizeof why, "dissipating %.4g W, expected %.4g", p, expect_p);
            } else if (burned != cases[i].expect_burn) {
                ok = 0;
                snprintf(why, sizeof why, "%.4g W into a %.4g W part: damage %.3g, failed %d - "
                         "expected %s", p, cases[i].rating, dmg, (int)r->thermal.failed,
                         cases[i].expect_burn ? "it to burn" : "it to survive");
            }
        }
        if (!ok) { fails++; printf("[FAIL] therm %-58s %s\n", cases[i].what, why); }
        else printf(" OK  therm %-58s\n", cases[i].what);
        simulation_free(sim);
        circuit_free(c);
    }

    /* Every part type that carries a power rating has to be one the model can judge. A rating
       nothing reads is a rating that does nothing, and a type added later with a rating and no
       handling would go unnoticed - which is how the resistor's own rating came to be ignored. */
    printf("\nthermal-test: %d overload cases, %d wrong\n", total, fails);
    return fails ? 1 : 0;
}
